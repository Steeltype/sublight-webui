...
export function ensureProcess(session, ws) {
  ...
  proc.stdout.on('data', (chunk) => {
    ...
    for (const line of lines) {
      ...
      // Control-channel messages are internal plumbing (interrupt acks,
      // SDK init handshakes, etc.) and must not surface in the UI stream.
      // We log them for debugging and move on.
      if (event.type === 'control_response' || event.type === 'control_request' || event.type === 'control_cancel_request') {
        logToSession(session, { type: 'control', event });
        continue;
      }
      ...
      if (event.type === 'result') {
        ...
        // The turn ended — whether naturally or because Claude honored our
        // interrupt. Either way, clear the interrupt bookkeeping so the
        // next Stop click is armable.
        if (session.interruptFallback) {
          clearTimeout(session.interruptFallback);
          session.interruptFallback = null;
        }
  ...
  proc.on('close', (code) => {
    ...
    const wasAborting = !!session.aborting;
    session.aborting = false;
    session.status = 'idle';

    // Abandoned interrupt bookkeeping — the process is gone, no response is
    // coming, and the fallback (if any) already fired to get us here.
    if (session.interruptFallback) {
      clearTimeout(session.interruptFallback);
      session.interruptFallback = null;
    }
    ...
    logToSession(session, { type: 'process_exit', code, aborted: wasAborting });

    if (wasBusy) {
      session.unread = true;
      if (wasAborting) {
        // User-initiated stop that couldn't be handled in-band. The
        // process died with SIGTERM (exit 143) but resumeFromClaudeSession
        // is already set, so the next message respawns with --resume and
        // the conversation picks up where it left off. Don't surface
        // "exited with code 143" as an error — the user stopped it.
