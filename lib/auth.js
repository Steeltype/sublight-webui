// Auth + audit helpers. The active token is resolved live on every request
// so changes written via saveSettings (first-run setup, regenerate) take
// effect immediately. The SUBLIGHT_TOKEN env var, if set, always wins.
...
/** Current auth token, or null if auth is disabled. */
export function getAuthToken() {
  return process.env.SUBLIGHT_TOKEN || settings.current.token;
}
